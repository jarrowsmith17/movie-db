...
const getTopRatedMovies = async () => {
  // We use the 'discover' endpoint to apply specific "widely watched" criteria
  // Threshold: vote_count.gte=10000 ensures only films with significant viewership are included
  const pageRequests = [1, 2, 3, 4, 5].map(page =>
    fetch(
      `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.TMDB_API_KEY}&language=en-GB&sort_by=vote_average.desc&vote_count.gte=10000&page=${page}&region=GB`,
      { next: { revalidate: 86400 } }
    ).then(res => res.json())
  );

  const pages = await Promise.all(pageRequests);
  // Merge results and take the top 100
...
export default async function TopRatedPage() {
  ...
  // Consistent rating color logic from the movie detail pages
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-10 mt-12">
        <header className="mb-12 border-l-4 border-yellow-500 pl-6">
          ...
          <p className="text-gray-500 mt-2 font-medium max-w-2xl">
            Highest rated films with at least <span className="text-yellow-500">10,000+ ratings</span> to ensure only the most watched and loved classics make the cut.
          </p>
        </header>
